...
;; This program connects the database server through Ruby's DBI,
...
(defun edbi:disconnect-d (conn)
  ...
  ;; TODO kill process
  (when (and edbi:connection
             (zerop (process-exit-status (jsonrpc--process edbi:connection))))
    (jsonrpc-shutdown edbi:connection)))
;; (epc:stop-epc conn))

(defun edbi:status-info-d (conn)
  "Return a list of `err' code, `errstr' and `state'."
  (edbi:async-request 'status (list nil)
                      ;; :success-fn (jsonrpc-lambda (_) (message "TODO: return"))
                      ))
...
(defun edbi:type-info-all-d (conn)
  ...
  (edbi:async-request 'type_info_all (list nil)
                      ;; :success-fn (jsonrpc-lambda (_) (message "TODO: return"))
                      ))
